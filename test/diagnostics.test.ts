...
describe("formatHypaDiagnostics", () => {
  it("renders five sections in order for a healthy snapshot", () => {
    ...
    const inputLine = text.indexOf("  input:", rewriteIdx)
    const commandLine = text.indexOf("  command:", rewriteIdx)
    const outcomeLine = text.indexOf("  outcome:", rewriteIdx)
    const timestampLine = text.indexOf("  timestamp:", rewriteIdx)
    assert.ok(inputLine > rewriteIdx)
    assert.ok(commandLine > inputLine)
    assert.ok(outcomeLine > commandLine)
    assert.ok(timestampLine > outcomeLine)
